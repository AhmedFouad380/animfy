...
@section('content')
  ...
  <div class="container" style="min-height: 70vh; padding-top: 40px; padding-bottom: 60px;">
    ...
    <div class="tabs-container"
      style="display: flex; gap: 15px; margin-bottom: 35px; border-bottom: 2px solid #FFF0E6; padding-bottom: 0px; flex-wrap: wrap;">
      <button class="tab-btn active" onclick="switchMyTab(event, 'courses')"
        style="background: none; border: none; padding: 12px 20px; font-size: 1.1rem; font-weight: 700; color: #da6319; border-bottom: 3px solid #da6319; cursor: pointer; transition: 0.3s;">
        {{ app()->getLocale() === 'ar' ? 'دوراتي التدريبية' : 'My Courses' }}
      </button>
      <button class="tab-btn" onclick="switchMyTab(event, 'addons')"
        style="background: none; border: none; padding: 12px 20px; font-size: 1.1rem; font-weight: 600; color: #555; cursor: pointer; transition: 0.3s;">
        {{ app()->getLocale() === 'ar' ? 'الإضافات المشتراة' : 'My Addons' }}
      </button>
      <button class="tab-btn" onclick="switchMyTab(event, 'objects')"
        style="background: none; border: none; padding: 12px 20px; font-size: 1.1rem; font-weight: 600; color: #555; cursor: pointer; transition: 0.3s;">
    ...
    <div class="courses-grid">
      ...
      <div id="courses-section" class="tab-section-content" style="display: block;">
        <div class="course-container">
          @forelse($coursesEnrollments as $enrollment)
            ...
            @if($course)
              <div class="course-card">
                ...
                <img class="course-img" src="{{ asset('storage/' . $course->thumbnail) }}"
                ...
                <a href="{{ route('classroom', $course->id) }}" class="watch-btn"
                  style="text-align: center; display: block; text-decoration: none; line-height: 2.5; font-weight: 600;">
          ...
          @empty
            <div
              style="grid-column: 1 / -1; text-align: center; padding: 60px 20px; background: #fff; border-radius: 20px; border: 1px solid #da6319; width: 100%; box-shadow: 0 5px 15px rgba(0,0,0,0.05);">
              ...
              <a href="{{ route('home') }}" class="buy-btn"
                style="display: inline-block; text-decoration: none; padding: 12px 30px; font-weight: 600; line-height: 1.5; width: auto; max-width: 320px; margin: 0 auto; background: #da6319; color: #fff; border-radius: 10px; border: none;">
      ...
      <div id="addons-section" class="tab-section-content" style="display: none;">
        <div class="course-container">
          @forelse($addonsEnrollments as $enrollment)
            ...
            @if($addon)
              <div class="course-card">
                <img class="course-img" src="{{ asset('storage/' . $addon->thumbnail) }}"
                  onerror="this.src='{{ asset('imgs/courses-thumbnails/blender-thumbnail.jpg') }}'" alt="Addon Thumbnail" />
      ...
      <div id="objects-section" class="tab-section-content" style="display: none;">
        <div class="course-container">
          @forelse($objectsEnrollments as $enrollment)
            ...
            @if($object)
              <div class="course-card">
                <img class="course-img" src="{{ asset('storage/' . $object->thumbnail) }}"
                  onerror="this.src='{{ asset('imgs/courses-thumbnails/blender-thumbnail.jpg') }}'"
                  alt="3D Object Thumbnail" />
